...
runs = [spec30,spec45,spec60]#[m18pol30, m18pol45, m18pol60, m18pol75]#[m1875]#
labels=["30 degrees","45 degrees","60 degrees"]#["30 degrees", "45 degrees", "60 degrees", "75 degrees"]#["75 degrees"]#

# Plot BRIDF data
sample_name="Spectralon"
plot_runs(runs, title=sample_name+", 500 nm", log=True, labels=labels, errorbars=True,legend_loc=6)
t0=time.time()

# Fit data
fit_params = fit_parameters(get_independent_variables_and_relative_intensities(runs),p0=[0.9,1.3,.3],average_angle=4., precision=-1,use_errs=True,use_spike=False)
...
plot_TSTR_fit(30., 1., fit_params, color="k", average_angle=4., precision=-1)
plot_TSTR_fit(45., 1., fit_params, color="k", average_angle=4., precision=-1)
plot_TSTR_fit(60., 1., fit_params, color="k", average_angle=4., precision=-1)
